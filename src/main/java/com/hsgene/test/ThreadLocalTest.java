...
public class ThreadLocalTest {
     ...
     * 源码：创建一个ThreadLocal对象(那么该对象就属于当前线程)--该对象的内部类对象ThreadLocalMap是Thread的成员，
     ...
     * 需要时通过ThreadLocal的get()方法获取该变量(需要强转)
     * 总结：使用ThreadLocal保证了当前用户的线程只能使用自己的变量，因为同一时刻一个线程只会被一个请求使用，不会跨线程使用变量，
     * 这样同步也只局限于线程内部，同时多线程运行时就能既同步性能影响也不大
     * 核心：同步会影响性能，使用ThreadLocal只需要线程同步，而单例则需要进程或整个应用都同步；绑定线程和变量，
     * 这样就不需要反复创建对象(例数据源)，需要时从线程中取，当不再被使用时，因为继承自弱引用，其还会被自动回收；一个线程创建多个ThreadLocal对象，即可以绑定多个变量
     */
    @Test
